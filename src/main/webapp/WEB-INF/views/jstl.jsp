...
<html>
	...
	<body>
		...
		표기법으로 간단히 태그화하여 표준으로 제공하는 것.
		
		(if, for, 배열/컬렉션 길이, 문자열 치환, 숫자 데이터 형식 변경, 데이터파싱, scope 변수 선언 등)
		</pre>
		
		<h3>JSTL 라이브러리 등록 방법</h3>
		
		<ol>
			<li>http://tomcat.apache.org/download-tablibs.cgi 접속</li>
		...
		- c:remove 속성
		1) var : 삭제할 변수명(key)
		2) scope : 삭제한 내장 객체 범위(기본값 : 모든 범위)
		</pre>
		...
		* 주의 사항 *
		1) test 속성값 작성은 무조건 EL 구문이어야만 한다.
		...
		<%--
		 ...
		 <c:if test='${sessionScope == "session" }'>
		...
		c:choose : when, otherwise를 감싸는 태그
				(현재 태그 내부에 if~else, if~esle를 작성하겠다.)
